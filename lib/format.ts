...
export function freshnessState(updatedAt: string, now = new Date("2026-09-01T22:00:00+05:00")) {
  ...
  if (ageDays <= 7) return { label: "Обновлено недавно", tone: "success" as const, ageDays };
  if (ageDays <= 30) return { label: "Лучше уточнить", tone: "warning" as const, ageDays };
  return { label: "Давно не обновлялось", tone: "danger" as const, ageDays };
}

export function responseLabel(minutes: number | null, sampleSize: number) {
  if (minutes === null || sampleSize < 5) return "Пока мало ответов для оценки";
