...
export function getOfferDetails(service: Service): OfferDetails {
  if (service.sourceUrl) {
    ...
    return {
      ...
      facts: [
        { label: copy("Источник", "Manba"), value: service.sourcePlatform === "olx" ? copy("Публичное объявление OLX.uz", "OLX.uz dagi ochiq e’lon") : service.sourcePlatform === "telegram" ? copy("Публикация в группе Event Uzbekistan", "Event Uzbekistan guruhidagi e’lon") : copy("Официальный сайт автора", "Muallifning rasmiy sayti"), href: service.sourceUrl },
